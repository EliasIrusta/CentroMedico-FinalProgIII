...
import { message, Space, Table, Modal, TimePicker, DatePicker, Select, Button } from 'antd'
...
function ModificarTurno() {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Button onClick={() => handleChangeEstado(record._id)} type="primary" icon={<CheckOutlined />} />
        </Space>
      ),
    },
    {
    ...
    try {
      ...
      const turnosConMedico = await Promise.all(response.map(async (turno) => {
        if (turno.medico_id) {
          ...
          if (medico) {
            return { ...turno, medico: { nombre: medico.firstName, apellido: medico.lastName } };
          }
        }
        return turno;
      }));
  ...
  const horaChange = (date, dateString) => {
    console.log('Valor de dateString:', dateString); // Agregar este console.log para verificar el valor de dateString
    setHora(dateString); 
}
  ...
  return (
    <div>
      ...
      <Table columns={columns} dataSource={turnos} style={{ textTransform: 'capitalize' }} />
