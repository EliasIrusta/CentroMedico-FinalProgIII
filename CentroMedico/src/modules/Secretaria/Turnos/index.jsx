...
import { Form, Input, DatePicker, Button, Select, AutoComplete, Card } from 'antd';
...
function Turnos() {
  ...
  const listarUsuarios = async () => {
    try {
      ...
      const especialidadesUnicas = [...new Set(response.map((item) => item.especialidad))];
  ...
  const onSelectEspecialidad = (especialidad) => {
    ...
    const usuariosFiltrados = usuarios.filter((usuario) => usuario.especialidad === especialidad);
  ...
  const onSelect = (data) => {
    const pacienteSeleccionado = pacientes.find((elemento) => elemento.dni === data);
    ...
    if (pacienteSeleccionado) {
      ...
      // setIdPaciente(pacienteSeleccionado._id);
    ...
    <div>
      ...
      >
        ...
        <Form.Item
          ...
          labelCol={{ span: 12 }}
          wrapperCol={{ span: 12 }}
        > <span style={{ border: '1px solid #d9d9d9', borderRadius: '5px', padding: '5px', textTransform: 'capitalize' }}>
            {nombrePaciente && `${nombrePaciente.firstName} ${nombrePaciente.lastName}`}
          </span>
      ...
      <Form.Item
        ...
        wrapperCol={{ span: 12 }}
      >
        <Select placeholder="Selecciona una especialidad" onSelect={onSelectEspecialidad}>
